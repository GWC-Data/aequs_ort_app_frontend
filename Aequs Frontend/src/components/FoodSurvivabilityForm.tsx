...
interface FormRow {
    ...
    visual: string;
    t0Picture: string;
    afterHeatSoakCosmetic: string;
    afterHeatSoakNonCosmetic: string;
    afterFootSurvivabilityCosmetic: string;
    afterFootSurvivabilityNonCosmetic: string;
...
interface FootSurvivabilityFormProps {
    formData: FormData;
...
export default function FootSurvivabilityForm({
    formData,
    updateFormField,
    updateRowField,
    addRow
}: FootSurvivabilityFormProps) {
    return (
        <div className="p-8 bg-gray-50 min-h-screen">
            <div className="max-w-full mx-auto">
                <h2 className="text-3xl font-bold text-gray-900 mb-8">Foot Survivability Test</h2>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-300">
                                    ...
                                    <th className="px-4 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap">T0 Picture</th>
                                    <th className="px-4 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap">After Heat Soak (Cosmetic)</th>
                                    <th className="px-4 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap">After Heat Soak (Non-Cosmetic)</th>
                                    <th className="px-4 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap">After Foot Survivability (Cosmetic)</th>
                                    <th className="px-4 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap">After Foot Survivability (Non-Cosmetic)</th>
                                ...
                                {formData.rows.map((row, index) => (
                                    <tr key={row.id} className={index % 2 === 0 ? "bg-white hover:bg-gray-50" : "bg-gray-50 hover:bg-gray-100"}>
                                        ...
                                        <td className="px-4 py-4">
                                            <input
                                                value={row.t0Picture}
                                                onChange={(e) => updateRowField(row.id, 't0Picture', e.target.value)}
                                        ...
                                        <td className="px-4 py-4">
                                            <input
                                                value={row.afterHeatSoakCosmetic}
                                                onChange={(e) => updateRowField(row.id, 'afterHeatSoakCosmetic', e.target.value)}
                                                className="w-full min-w-[120px] px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                                placeholder="Value"
                                            />
                                        </td>
                                        <td className="px-4 py-4">
                                            <input
                                                value={row.afterHeatSoakNonCosmetic}
                                                onChange={(e) => updateRowField(row.id, 'afterHeatSoakNonCosmetic', e.target.value)}
                                                className="w-full min-w-[120px] px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                                placeholder="Value"
                                            />
                                        </td>
                                        <td className="px-4 py-4">
                                            <input
                                                value={row.afterFootSurvivabilityCosmetic}
                                                onChange={(e) => updateRowField(row.id, 'afterFootSurvivabilityCosmetic', e.target.value)}
                                                className="w-full min-w-[120px] px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                                placeholder="Value"
                                            />
                                        </td>
                                        <td className="px-4 py-4">
                                            <input
                                                value={row.afterFootSurvivabilityNonCosmetic}
                                                onChange={(e) => updateRowField(row.id, 'afterFootSurvivabilityNonCosmetic', e.target.value)}
                                                className="w-full min-w-[120px] px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                                placeholder="Value"
                                            />
                                        </td>
