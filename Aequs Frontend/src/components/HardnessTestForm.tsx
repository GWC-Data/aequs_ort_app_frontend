...
interface HardnessTestFormProps {
    formData: FormData;
...
export default function HardnessTestForm({
    formData,
    updateFormField,
    updateRowField,
    addRow
}: HardnessTestFormProps) {
    return (
        <div className="p-8 bg-gray-50 min-h-screen">
            <div className="max-w-full mx-auto">
                <h2 className="text-3xl font-bold text-gray-900 mb-8">Hardness Test</h2>
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="bg-gradient-to-r from-gray-50 to-gray-100 border-b border-gray-300">
                                    ...
                                    <th className="px-4 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap">Scale</th>
                                    <th className="px-4 py-4 text-left text-xs font-bold text-gray-700 uppercase tracking-wider whitespace-nowrap">Hardness</th>
                                ...
                                {formData.rows.map((row, index) => (
                                    <tr key={row.id} className={index % 2 === 0 ? "bg-white hover:bg-gray-50" : "bg-gray-50 hover:bg-gray-100"}>
                                        ...
                                        <td className="px-4 py-4">
                                            <input
                                                value={row.scale}
                                                onChange={(e) => updateRowField(row.id, 'scale', e.target.value)}
                                        ...
                                        <td className="px-4 py-4">
                                            <input
                                                value={row.hardness}
                                                onChange={(e) => updateRowField(row.id, 'hardness', e.target.value)}
                                                className="w-full min-w-[100px] px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
