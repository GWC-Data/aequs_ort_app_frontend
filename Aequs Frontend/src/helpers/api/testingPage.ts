...
const sanitizePayload = (payload: TestingPartData): TestingPartData => {
  ...
  const sanitized: TestingPartData = {
    ...payload,
    lastUpdated: payload.lastUpdated ?? new Date().toISOString(),
  };

  // Only include parts when caller provides them; otherwise leave unchanged on the server
  if (payload.parts !== undefined) {
    sanitized.parts = ensureSerializable(payload.parts);
  }

  // Preserve machineDetails unless explicitly provided
  if (payload.machineDetails !== undefined) {
    sanitized.machineDetails = ensureSerializable(payload.machineDetails);
  }
