...
//   const normalizedGlobalCustomData: Record<string, string> = globalCustomColumnData;
...
//     const images = Array.isArray((part as any)?.customImages) ? (part as any).customImages : [];
...
//       const combined = labelMatch.length > 0 ? labelMatch : parseImageList(fallbackVal);
...
//       const hasT0Label = sortedData.some((entry) => (entry.label ?? "").toString().toLowerCase().includes("t0"));
...
//       const hasT0Value = sortedData.some((entry) => (entry.checkpointValue ?? entry.checkpoint) === t0Value);

//       if (!hasT0Label && !hasT0Value) {
//         const syntheticT0: ChamberCheckpointDataEntry = {
//           label: "T0",
//           checkpointIndex: 0,
//           checkpointValue: t0Value,
//           testDate: part.loadedAt ?? load.loadedAt ?? null,
//           customData: Object.keys(t0CustomData).length > 0 ? t0CustomData : undefined,
//           status: part.scanStatus ?? load.status ?? null
//         };
//         sortedData.unshift(syntheticT0);
//       }
...
//           const fallbackValue = inlineValue ?? t0Fallback ?? globalCustomColumnData[fallbackKey] ?? null;
...
//         const t0RowCustomData: Record<string, string> = {};
//         customColumnIds.forEach((columnId) => {
//           const fallbackKey = `${partIndex}-0-${columnId}`;
//           const rawVal = t0CustomData[columnId] ?? normalizedGlobalCustomData[fallbackKey] ?? null;
//           const normalized = normalizeCustomValue(rawVal);
//           if (normalized) {
//             t0RowCustomData[columnId] = normalized;
//           }
//         });

//         const fallbackImage = firstCustomImage(t0RowCustomData ?? {});
...
//         partIndex,
//         checkpointIndex: 0,
//           customData: Object.keys(t0RowCustomData).length > 0 ? t0RowCustomData : undefined,
//       });
//     }
//   });
...
//     /^https?:\/\//.test(trimmed) ||
//     /^\/?uploads\//.test(trimmed) ||
//     /(\.)(png|jpe?g|webp|gif|bmp|tif|tiff|heic|heif)(\?|$)/.test(trimmed)
//   );
// };

// const containsImageLikeValue = (raw: string): boolean => {
//   if (!raw) return false;
//   try {
//     const parsed = JSON.parse(raw);
//     if (Array.isArray(parsed)) {
//       return parsed.some((entry) => typeof entry === "string" && isLikelyImagePath(entry));
//     }
//     if (typeof parsed === "string") {
//       return isLikelyImagePath(parsed);
//     }
//   } catch (_err) {
//     /* ignore */
//   }
//   return isLikelyImagePath(raw);
// };
...
// type HulkCropRect = { x: number; y: number; width: number; height: number };

// const HULK_IMAGE_CROPS: Record<string, HulkCropRect> = {
//   font: { x: 130, y: 60, width: 545, height: 495 },
//   front: { x: 130, y: 60, width: 545, height: 495 },
//   back: { x: 130, y: 60, width: 545, height: 495 },
//   e2: { x: 290, y: 500, width: 640, height: 160 },
//   e4: { x: 290, y: 500, width: 640, height: 160 },
//   e6: { x: 290, y: 500, width: 640, height: 160 },
//   e8: { x: 290, y: 500, width: 640, height: 160 }
// };

// const getHulkCropRect = (projectName?: string, imageLabel?: string): HulkCropRect | null => {
//   const normalizedProject = (projectName ?? "").trim().toLowerCase();
//   if (!normalizedProject.includes("hulk")) {
//     return null;
//   }

//   const normalizedLabel = (imageLabel ?? "").trim().toLowerCase();
//   if (!normalizedLabel) {
//     return null;
//   }

//   return HULK_IMAGE_CROPS[normalizedLabel] ?? null;
// };

// const cropBase64Image = async (base64: string, mime: string, rect: HulkCropRect): Promise<string> => {
...
//     const img = new Image();
//     img.onload = () => {
//       const safeWidth = Math.max(1, Math.floor(img.width));
//       const safeHeight = Math.max(1, Math.floor(img.height));
//       const sx = Math.max(0, Math.min(rect.x, safeWidth));
//       const sy = Math.max(0, Math.min(rect.y, safeHeight));
//       const sWidth = Math.max(1, Math.min(rect.width, safeWidth - sx));
//       const sHeight = Math.max(1, Math.min(rect.height, safeHeight - sy));
...
//       resolve(commaIndex === -1 ? base64 : dataUrl.slice(commaIndex + 1));
//     };
...
//   imageSrc: string,
//   context?: { projectName?: string; imageLabel?: string }
...
//   const cropRect = getHulkCropRect(context?.projectName, context?.imageLabel);
//   const mime = result.mime || "image/png";
//   const processedBase64 = cropRect
//     ? await cropBase64Image(result.base64, mime, cropRect)
//     : result.base64;
...
//   console.log(load)
//   const hydrated = hydrateChamberLoadData(load ?? {});
//   const projectName = hydrated.machineDetails?.project ?? "";
...
//         const def = customColumnDefs[idx];
//         const isImageCol = def?.type === "image" || containsImageLikeValue(val);
//         return isImageCol ? "" : val;
//       });
...
//         const isImageCol = def?.type === "image" || containsImageLikeValue(raw);
//         if (!isImageCol) {
//           return;
//         }
//         const parsed = (() => {
...
//           def?.label
//           ?? def?.name
//           ?? customColumnIds[customIndex]
//           ?? customColumnLabels[customIndex]
//           ?? ""
//         ).toString();
//         enqueueImagesForRow(row, images, zeroIndex, cellIndex, columnLabel);
//       });
//     };
...
//         const queueImage = (imageSrc: string | undefined, colIdx: number, columnLabel: string) => {
...
//             def?.label
//             ?? def?.name
//             ?? consolidatedCustomIds[customIndex]
//             ?? consolidatedCustomLabels[customIndex]
//             ?? ""
//           ).toString();
//           images.forEach((src) => queueImage(src, 4 + customIndex, columnLabel));
//         });
...
const buildCustomColumnRegistry = (load: ChamberLoadData) => {
  ...
  const IMAGE_COL_ORDER = [
    "FRONT", "BACK", "E2", "E4", "E6", "E8", "COSMETIC_IMAGE", "NON_COSMETIC_IMAGE",
  ];

  const rawDefinitions = Array.from(registry.values()).map((definition) => {
    const label = typeof definition.label === "string" && definition.label.trim()
      ? definition.label.trim()
      : definition.name ?? definition.id;
    const name = typeof definition.name === "string" && definition.name.trim()
  ...
  // Non-image columns first (original order), then image columns in standard order
  const nonImageDefs = rawDefinitions.filter((d) => d.type !== "image");
  const imageDefs = rawDefinitions.filter((d) => d.type === "image");
  imageDefs.sort((a, b) => {
    const ai = IMAGE_COL_ORDER.indexOf((a.name || a.id || "").toUpperCase());
    const bi = IMAGE_COL_ORDER.indexOf((b.name || b.id || "").toUpperCase());
    return (ai === -1 ? IMAGE_COL_ORDER.length : ai) - (bi === -1 ? IMAGE_COL_ORDER.length : bi);
  });
  const definitions = [...nonImageDefs, ...imageDefs];
      ...
      customColumnIds.forEach((columnId, idx) => {
        ...
        const labelMatch = images
          .filter((img) => {
            ...
            const normalizedId = (columnId || "").toLowerCase();
            const normalizedName = (def?.name || def?.label || "").toString().trim().toLowerCase();
            return lbl === normalizedId || (!!normalizedName && lbl === normalizedName);
          })
          .map((img) => (img as any)?.path)
          .filter((p): p is string => typeof p === "string" && p.trim().length > 0);
        const isLikelyImageColumn =
          (def?.type === "image") ||
          labelMatch.length > 0;
        if (!isLikelyImageColumn) {
          return;
        }
        const fallbackKey = `${partIndex}-0-${columnId}`;
        const fallbackVal = normalizedGlobalCustomData[fallbackKey];

        // If no loading image and no checkpoint-0 data, pick the first available
        // checkpoint image for this column (e.g. images uploaded at Final Test).
        let anyCheckpointVal: string | null = null;
        if (labelMatch.length === 0 && !fallbackVal) {
          const prefix = `${partIndex}-`;
          const suffix = `-${columnId}`;
          for (const [k, v] of Object.entries(normalizedGlobalCustomData)) {
            if (k.startsWith(prefix) && k.endsWith(suffix) && v) {
              anyCheckpointVal = v;
              break;
            }
          }
        ...
        const combined = labelMatch.length > 0
          ? labelMatch
          : toImageArray(fallbackVal ?? anyCheckpointVal);
        if (combined.length > 0) {
          customData[columnId] = JSON.stringify(combined);
        }
      });
    ...
    hydrated.parts?.forEach((part, partIndex) => {
      ...
      if (Array.isArray(checkpoints) && checkpoints.length > 0) {
        ...
        checkpoints.forEach((entry, entryIndex) => {
          ...
          const isT0Entry =
            (checkpointLabel ?? "").toString().toLowerCase().includes("t0") ||
            String(checkpointValue ?? "").toLowerCase() === "t0" ||
            checkpointIndex === 0;
          if (
            (checkpointLabel ?? "").toString().toLowerCase().includes("t0") ||
            checkpointIndex === 0
          ...
          const rawCustomValues = customColumnIds.map((columnId, customIndex) => {
            const inlineValue = (entry as any)?.customData?.[columnId] ?? null;
            const inlineNormalized = normalizeCustomValue(inlineValue);
            const fallbackKey = `${partIndex}-${checkpointIndex}-${columnId}`;
            const t0Fallback = isT0Entry ? t0CustomData[columnId] ?? null : null;
            const fallbackValue = inlineNormalized ?? t0Fallback ?? normalizedGlobalCustomData[fallbackKey] ?? null;
            const normalized = normalizeCustomValue(fallbackValue);
            return normalized ?? "";
          });
          const displayCustomValues = rawCustomValues.map((val, idx) => {
