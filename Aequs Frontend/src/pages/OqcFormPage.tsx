...
const OQCSystem = () => {
  ...
  const loadExcelData = async () => {
    try {
      ...
      const oqcConfigs = await apiService.getOqcForms();
      ...
      oqcConfigs.forEach(item => {
        const addOption = (col: string, val: string | undefined) => {
          if (!val) return;
          const trimmed = val.trim();
          if (trimmed && !dropdownData.dropdownOptions[col].includes(trimmed)) {
            dropdownData.dropdownOptions[col].push(trimmed);
          }
        };

        addOption("ASSEMBLY_ANO_OPTIONS", item.assemblyAnoOptions);
        addOption("SOURCE_OPT", item.sourceOpt);
        addOption("SOURCE_AND_TYPE", item.sourceAndType);
        addOption("PRODUCT_OPTIONS", item.productOptions);
        addOption("PRODUCT_PROJECT", item.productProject);
        addOption("PROJECT_OPTIONS", item.projectOptions);
        addOption("COLOUR_OPTIONS", item.colourOptions);
        addOption("REASON_OPTIONS", item.reasonOptions);

        if (item.assemblyAnoOptions && item.anoCode) {
          dropdownData.codeMappings["ASSEMBLY_ANO_OPTIONS"][item.assemblyAnoOptions.trim()] = item.anoCode.trim();
        }
        if (item.productOptions && item.productCode) {
          dropdownData.codeMappings["PRODUCT_OPTIONS"][item.productOptions.trim()] = item.productCode.trim();
        }
        if (item.projectOptions && item.projectCode) {
          dropdownData.codeMappings["PROJECT_OPTIONS"][item.projectOptions.trim()] = item.projectCode.trim();
        }
        if (item.colourOptions && item.colourCode) {
          dropdownData.codeMappings["COLOUR_OPTIONS"][item.colourOptions.trim()] = item.colourCode.trim();
        }
        if (item.reasonOptions && item.reasonCode) {
          dropdownData.codeMappings["REASON_OPTIONS"][item.reasonOptions.trim()] = item.reasonCode.trim();
        }

        const addUnique = (map: Record<string, string[]>, key: string | undefined, val: string | undefined) => {
          if (!key || !val) return;
          const k = key.trim();
          const v = val.trim();
          if (!map[k]) map[k] = [];
        ...
        if (item.productCode && item.projectOptions) {
          addUnique(dropdownData.productProjectsMappings, item.productCode, item.projectOptions);
        }
        if (item.projectOptions && item.sourceAndType) {
          addUnique(dropdownData.projectStageMappings, item.projectOptions, item.sourceAndType);
        }
        if (item.sourceAndType && item.sourceOpt) {
          addUnique(dropdownData.sourceMappings, item.sourceAndType, item.sourceOpt);
        }
      });

      // ── Fallback: supplement dropdowns from Test Allocation config ──
      // If no OQC Form Mappings data exists, pull unique product names
      // and process stages from the Test Allocation config so the
      // "Create New Ticket" dialog still has options to select from.
      try {
        const testAllocations = await apiService.getTestAllocations();
        if (testAllocations && testAllocations.length > 0) {
          // Supplement PRODUCT_OPTIONS with unique product names from Test Allocation
          testAllocations.forEach(ta => {
            if (ta.product && ta.product.trim()) {
              const prod = ta.product.trim();
              if (!dropdownData.dropdownOptions["PRODUCT_OPTIONS"].includes(prod)) {
                dropdownData.dropdownOptions["PRODUCT_OPTIONS"].push(prod);
              }
            }
            // Supplement ASSEMBLY_ANO_OPTIONS (stage) with processesStage from Test Allocation
            if (ta.processesStage && ta.processesStage.trim()) {
              const stage = ta.processesStage.trim();
              if (!dropdownData.dropdownOptions["ASSEMBLY_ANO_OPTIONS"].includes(stage)) {
                dropdownData.dropdownOptions["ASSEMBLY_ANO_OPTIONS"].push(stage);
              }
              // Also add to SOURCE_AND_TYPE so stages show up in the Stage dropdown
              if (!dropdownData.dropdownOptions["SOURCE_AND_TYPE"].includes(stage)) {
                dropdownData.dropdownOptions["SOURCE_AND_TYPE"].push(stage);
              }
            }
          });
        }
      } catch (testAllocError) {
        console.warn("Could not load Test Allocation configs for fallback:", testAllocError);
      }
      ...
      console.log("Dynamic dropdown data loaded from Database:", {
      ...
      if (Object.keys(dropdownData.dropdownOptions).every(k => dropdownData.dropdownOptions[k].length === 0)) {
        showNotification("info", "No dropdown options found. Please add data in Configuration page.");
      }
      
    } catch (error) {
      console.error("Error loading configs from Database:", error);
      showNotification("error", "Failed to load dropdown options from Database config");
    }
  };
