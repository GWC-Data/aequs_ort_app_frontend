...
const OQCSystem = () => {
  ...
  //   setIsLoading(true);
  //   try {
  //     const response = await apiService.getAllOqcForms();

  //     // Transform backend data to match your TestRecord interface
  //     const transformedTickets: TestRecord[] = response.map((item: any) => ({
  //       id: item.id || item._id || Date.now(), // Use backend ID
  //       ticketCode: item.ticketCode,
  //       totalQuantity: item.totalQty, // Note: field name is totalQty in backend
  //       anoType: item.processStage, // Map processStage to anoType
  //       source: item.source,
  //       reason: item.reason,
  //       project: item.project,
  //       build: item.build,
  //       colour: item.colour,
  //       dateTime: item.date || item.CreatedAt || new Date().toISOString().split('T')[0],
  //       status: 'In-Progress', // You might want to add status field to backend
  //       sessions: [], // Empty array, sessions will be loaded separately
  //       createdAt: item.CreatedAt || item.createdAt || new Date().toISOString(),
  //       oqcApproved: false // Default, add this field to backend if needed
  //     }));
...
//     console.log("Is array?", Array.isArray(ticketsResponse));
//     console.log("First item:", ticketsResponse[0]);
//     console.log("Keys of first item:", Object.keys(ticketsResponse[0]));
    
//     // Fetch scanned parts (temporarily disabled or handled separately)
//     let scannedPartsResponse = [];
//     try {
//       scannedPartsResponse = await apiService.getAllScannedParts();
//       console.log("Scanned parts response:", scannedPartsResponse);
//     } catch (scannedError) {
//       console.warn("Could not fetch scanned parts:", scannedError);
//       // Continue without scanned parts
//     }
    
//     // Create a map of ticketId to scanned parts
//     const scannedPartsMap = new Map();
//     if (Array.isArray(scannedPartsResponse)) {
//       scannedPartsResponse.forEach((scannedData: any) => {
//         if (scannedData.ticketId) {
//           scannedPartsMap.set(scannedData.ticketId, scannedData.parts || []);
//         }
//       });
//     }

//     // Transform backend data to match your TestRecord interface
//     const transformedTickets: TestRecord[] = ticketsResponse.map((item: any) => {
//       console.log("Processing item:", item);
      
//       // Get the correct ID - backend uses 'Id' (uppercase I)
//       const ticketId = item.Id || item.id || Date.now();
      
//       // Convert scanned parts to session format
//       const scannedParts = scannedPartsMap.get(ticketId) || [];
//       const sessions: ScanSession[] = [];
      
//       if (scannedParts.length > 0) {
//         // Group parts into sessions if needed, or create one session
//         sessions.push({
//           id: `session-${ticketId}-1`,
//           sessionNumber: 1,
//           timestamp: new Date().toISOString(),
//           parts: scannedParts.map((part: any, idx: number) => ({
//             id: `part-${ticketId}-${idx}`,
//             partNumber: part.part || part.partNumber || `PART-${String(idx + 1).padStart(3, '0')}`,
//             serialNumber: part.serialNumber || `SN${String(idx + 1).padStart(3, '0')}`,
//             scanStatus: part.status || part.scanStatus || 'Cosmetic OK',
//             scannedAt: part.scannedAt || new Date().toISOString(),
...
//       return {
//         id: ticketId, // Use the correct ID
//         ticketCode: item.ticketCode,
...
//         build: item.build,
//         colour: item.colour,
//         dateTime: item.date ? new Date(item.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
//         status: 'In-Progress',
//         sessions: sessions,
//         createdAt: item.createdAt || item.CreatedAt || new Date().toISOString(),
//         oqcApproved: false
...
//               console.log(`Parsed ${partsArray.length} parts from JSON string`);
//             } catch (e) {
...
//           scannedPartsMap.set(scannedData.ticketId, partsArray);
//         }
//       });
//     }

//     console.log("Scanned parts map:", scannedPartsMap);
...
//       // Get scanned parts for this ticket
//       let scannedParts = scannedPartsMap.get(ticketId);
      
//       // If not found by ticketId, try by ticketCode
//       if (!scannedParts || !Array.isArray(scannedParts) || scannedParts.length === 0) {
//         const scannedPartsByCode = Array.from(scannedPartsMap.entries()).find(
//           ([, partsData]) => {
//             // Check if any scanned data has this ticketCode
//             if (Array.isArray(partsData) && partsData.length > 0) {
//               // Look for ticketCode in the parts data or in the scannedData object
//               return scannedPartsResponse.find(sd => 
//                 sd.ticketCode === item.ticketCode || 
//                 sd.ticketId === ticketId
//               );
//             }
//             return false;
//           }
//         );
//         if (scannedPartsByCode) {
//           scannedParts = scannedPartsByCode[1];
//         }
//       }
      
//       // Ensure scannedParts is an array
//       if (!Array.isArray(scannedParts)) {
//         scannedParts = [];
//       }
...
//       if (scannedParts.length > 0) {
//         console.log(`Found ${scannedParts.length} scanned parts for ticket ${ticketId}:`, scannedParts);
...
//             partNumber: part.part || part.partNumber || `PART-${String(idx + 1).padStart(3, '0')}`,
//             serialNumber: part.serialNumber || `SN${String(idx + 1).padStart(3, '0')}`,
//             scanStatus: (part.status || part.scanStatus || 'Cosmetic OK') as 'Cosmetic OK' | 'Cosmetic Not OK',
//             scannedAt: part.scannedAt || new Date().toISOString(),
//             location: part.location || 'home'
//           })),
//           submitted: true,
...
const fetchTicketsFromBackend = async () => {
  ...
  try {
    ...
    const transformedTickets: TestRecord[] = ticketsResponse.map((item: any) => {
      ...
      const sortedScannedData = [...ticketScannedData].sort((a, b) =>
      ...
      const parseScannedPartsSessionNumber = (value: number | string | null | undefined): number | null => {
      ...
      sortedScannedData.forEach((scannedItem, index) => {
        const parsedSessionNumber = parseScannedPartsSessionNumber(scannedItem.session);
        const createdAt = scannedItem.createdAt || new Date().toISOString();
        const key = parsedSessionNumber !== null
        ...
        if (existingGroup) {
          ...
          if (new Date(createdAt).getTime() < new Date(existingGroup.firstCreatedAt).getTime()) {
      ...
      const sessionGroupsArray = Array.from(sessionGroupMap.values()).sort((a, b) => {
      ...
      sessionGroupsArray.forEach(group => {
        ...
        const effectiveSessionNumber = group.sessionNumber ?? allocateSessionNumber();
        const sessionTimestamp = sessionItems.reduce((earliest, item) => {
          const itemTime = new Date(item.createdAt || earliest).getTime();
          return itemTime < new Date(earliest).getTime() ? item.createdAt || earliest : earliest;
        }, sessionItems[0].createdAt || new Date().toISOString());

        const allPartsInSession: any[] = [];
        sessionItems.forEach(item => {
          if (item.partsArray && Array.isArray(item.partsArray)) {
            allPartsInSession.push(...item.partsArray);
          }
        });

        if (allPartsInSession.length === 0) {
          return;
        }
        ...
        sessions.push({
          ...
          parts: allPartsInSession.map((part: any, idx: number) => ({
            id: `part-${ticketId}-${effectiveSessionNumber}-${idx}`,
            partNumber: part.part || part.partNumber || `PART-${String(idx + 1).padStart(3, '0')}`,
            serialNumber: part.serialNumber || `SN${String(idx + 1).padStart(3, '0')}`,
            scanStatus: (part.status || part.scanStatus || 'Cosmetic OK') as 'Cosmetic OK' | 'Cosmetic Not OK',
            scannedAt: part.scannedAt || sessionTimestamp,
            location: part.location || 'home'
          })),
          submitted: true,
          submittedAt: sessionTimestamp,
          sentToORT: false
        });
      });
      ...
      if (sessions.length === 0 && ticketScannedData.length > 0) {
        // Fallback: combine all parts into one session
        const allParts: any[] = [];
        ticketScannedData.forEach(item => {
          if (item.partsArray && Array.isArray(item.partsArray)) {
            allParts.push(...item.partsArray);
          }
        });
        ...
        if (allParts.length > 0) {
          sessions.push({
            id: `session-${ticketId}-1`,
            sessionNumber: 1,
            timestamp: ticketScannedData[0].createdAt || new Date().toISOString(),
            parts: allParts.map((part: any, idx: number) => ({
      ...
      return {
        ...
        colour: item.colour,
        dateTime: item.date ? new Date(item.date).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        status: sessions.length > 0 ? 'Ready for OQC Approval' : 'In-Progress',
        sessions: sessions,
        createdAt: item.createdAt || item.CreatedAt || new Date().toISOString(),
        oqcApproved: false
      };
  ...
  const parseOrtSessionNumber = (value: number | string | null | undefined): number => {
    if (typeof value === 'number' && Number.isFinite(value)) {
      return value;
    }
  ...
  const mapOrtRecordToStage1Data = (record: BackendOrtRecord): ORTStage1Data => {
    ...
    const partNumbers = Array.isArray(detailBox.partNumbers)
      ? detailBox.partNumbers.filter((value): value is string => Boolean(value && value.trim()))
      : [];
    ...
    const mappedDetails = {
      ...
      dateShiftTime: detailBox.dateShiftTime ?? (record.date ? new Date(record.date).toLocaleString() : 'N/A'),
    ...
    return {
      ...
      scannedPartsId: typeof detailBox.scannedPartsId === 'number' ? detailBox.scannedPartsId : undefined,
      ...
      stage2Enabled: received === 'Yes' && (record.inventoryRemarks ?? '').trim() !== '' && !detailBox.movedToStage2,
  ...
  const loadORTStage1Data = async () => {
    try {
      ...
      const mapped = ortRecords.map((record: BackendOrtRecord) => mapOrtRecordToStage1Data(record));
  ...
  const handleReUploadFromORT = async (ortSession: ORTStage1Data) => {
    try {
      if (ortSession.ortRecordId) {
        await apiService.deleteOrtRecord(ortSession.ortRecordId);
      }

      let scannedPartsIdToDelete = ortSession.scannedPartsId;

      if (!scannedPartsIdToDelete && ortSession.ticketId) {
        const scannedRecords = await apiService.getScannedPartsByTicketSession(
          ortSession.ticketId,
          ortSession.sessionNumber
        );

        if (Array.isArray(scannedRecords) && scannedRecords.length > 0) {
          const recordCandidate = scannedRecords[0] as { Id?: number; id?: number };
          scannedPartsIdToDelete = recordCandidate.Id ?? recordCandidate.id;
        }
      }

      if (scannedPartsIdToDelete) {
        await apiService.deleteScannedParts(scannedPartsIdToDelete);
      }
    } catch (error) {
      console.error('Error preparing ORT re-upload:', error);
      showNotification('error', 'Failed to prepare session for re-upload. Please try again.');
      return;
    }
    ...
    if (existingTicket) {
      const updatedSessions = existingTicket.sessions.filter(
        session => session.sessionNumber !== ortSession.sessionNumber
      );
      ...
      const ortReason = reasonMatch ? reasonMatch[1].trim() : 'Unknown reason';
    ...
    setOrtStage1Data(prev => prev.filter(item => item.sessionId !== ortSession.sessionId));
  ...
  // Mass selection function for cosmetic status
  const handleMassStatusSelect = (status: 'Cosmetic OK' | 'Cosmetic Not OK') => {
    if (tempScannedParts.length === 0) {
      showNotification('error', 'No parts to update');
      return;
    }
    
    // Update all parts with the selected status
    const updatedParts = tempScannedParts.map(part => ({
      ...part,
      scanStatus: status
    }));
    
    setTempScannedParts(updatedParts);
    
    // Show notification based on how many parts were updated
    const changedCount = tempScannedParts.length;
    
    showNotification('success', 
      status === 'Cosmetic OK' 
        ? `All ${changedCount} parts marked as Cosmetic OK`
        : `All ${changedCount} parts marked as Cosmetic Not OK`
    );
  };

  // Clear all statuses function
  const handleClearAllStatus = () => {
    ...
    setTempScannedParts(updatedParts);
    showNotification('info', `Status cleared for all ${tempScannedParts.length} parts`);
  };
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showNotification('success', `Ticket ${newRecord.ticketCode} created successfully in database!`);
  ...
  //       location: 'home'
  //     })),
  //     submitted: true, // Auto-submit the session
  ...
  //   // Create ORT Lab data and save to localStorage
  //   const ortLabData = {
  //     id: sessionId,
  //     timestamp: new Date().toISOString(),
  ...
  //     parts: newSession.parts,
  //     project: selectedTicket.project,
  //     build: selectedTicket.build,
  //     colour: selectedTicket.colour,
  //     anoType: selectedTicket.anoType,
  //     source: selectedTicket.source,
  //     reason: selectedTicket.reason,
  //     oqcApprovedBy: selectedTicket.oqcApprovedBy,
  //     oqcApprovedAt: selectedTicket.oqcApprovedAt,
  //     submittedAt: newSession.submittedAt,
  //     totalParts: newSession.parts.length,
  //     serialNumber: newSession.parts.map(p => p.serialNumber).join(', '),
  //     partNumbers: newSession.parts.map(p => p.partNumber),
  //     rawBarcodeData: newSession.parts.map(p => `${p.serialNumber}:${p.partNumber}`).join('; '),
  //     submitted: true,
  //     totalQuantity: selectedTicket.totalQuantity
  //   };

  //   // Save to ORT Lab submissions
  ...
  //   showNotification('success',
  //     `Session ${sessionNumber} saved with ${tempScannedParts.length} parts! Redirecting to ORT Lab...`
  //   );
  ...
  if (Number.isNaN(ticketId)) {
    showNotification('error', 'Ticket is missing a valid database ID. Please refresh tickets and try again.');
    return;
  }
  ...
  try {
    ...
    const scannedPartsData = {
      ...
      ortReceivedStatus: 'Not Received' // Default status
    };

    // Call backend API to save scanned parts
    const backendResponse = await apiService.createScannedParts(scannedPartsData);
    ...
    // Also save to localStorage for ORT Lab (keep this for compatibility)
    const ortLabData = {
      id: sessionId,
      timestamp: new Date().toISOString(),
      ticketId: selectedTicket.id,
    ...
    showNotification('success',
      `Session ${sessionNumber} saved with ${tempScannedParts.length} parts! Data saved to database.`
    );
    ...
    showNotification('error', 'Failed to save scanned parts to database. Please try again.');
    setIsSaving(false);
  }
};
                    ...
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleMassStatusSelect('Cosmetic OK')}
                        ...
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => handleMassStatusSelect('Cosmetic Not OK')}
            ...
            {ticket.sessions.length === 0 ? (
              ...
              <div className="space-y-4">
                {ticket.sessions.map((session) => {
                  ...
                  const ortStatus = ortStatusMap[ortKey] as ORTStage1Data | undefined;
