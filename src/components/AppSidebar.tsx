...
const getUserTeam = () => {
  ...
  try {
    const userData = JSON.parse(user);
    return (
      userData.team?.toUpperCase() ||
      localStorage.getItem("userTeam")?.toUpperCase()
    );
  } catch {
...
export function AppSidebar() {
  ...
  if (userRole === "general") {
    return [{ title: "Home", url: "/", icon: Home }];
  }
  
    if (userTeam === "ALL") {
      // ALL Team - Show everything
      const items = [
      ...
      // Add User Management ONLY for admin with team "ALL"
      if (userRole === "admin") {
      ...
      // ORT Team Navigation - NO User Management even for admin
      const items = [
        { title: "Home", url: "/", icon: Home },
        { title: "ORT Lab", url: "/ort-lab-form", icon: Backpack },
        { title: "Test Allocation", url: "/settings", icon: TestTubeIcon },
      ...
      return items;
    } else if (userTeam === "OQC") {
      // OQC Team Navigation - Only OQC Form, NO User Management even for admin
      const items = [{ title: "OQC Form", url: "/oqcpage", icon: List }];
  ...
  const isActive = (path: string) => {
    ...
    // Planning Dashboard stays active when navigating into /form-default routes
    if (path === "/planning-detail") {
      return (
        currentPath === "/planning-detail" ||
        currentPath.startsWith("/form-default")
      );
    }
