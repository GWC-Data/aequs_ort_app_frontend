...
const LoadEquipmentModal: React.FC<LoadEquipmentModalProps> = ({
  ...
  // Excel-based test options filtered by equipment ID
  const [excelTestOptions, setExcelTestOptions] = useState<string[]>([]);
  ...
  // Load test options from Excel based on selected equipment ID
  const loadExcelTestOptions = async (machineId: string) => {
    try {
      ...
      const sheetName = "Machine_Details";
      const worksheet = workbook.Sheets[sheetName];
      if (!worksheet) {
        console.error("Machine_Details sheet not found");
        setExcelTestOptions([]);
        return;
      }
      ...
      const machineIdIdx = headers.findIndex((h) => {
        const lower = h.toString().toLowerCase().replace(/[\s_-]/g, "");
        return lower.includes("machineid") || lower.includes("equipmentid") || lower === "equipment";
      });
      const testNameIdx = headers.findIndex((h) => {
        const lower = h.toString().toLowerCase().replace(/[\s_-]/g, "");
        return lower.includes("testname") || lower === "test";
      });

      if (machineIdIdx === -1 || testNameIdx === -1) {
        console.error("Required columns not found in Machine_Details", headers);
        setExcelTestOptions([]);
        return;
      }
      ...
      for (let i = 1; i < jsonData.length; i++) {
        ...
        const rowMachineId = String(row[machineIdIdx] || "").trim();
        if (rowMachineId === machineId) {
          const testNameCell = String(row[testNameIdx] || "").trim();
          if (testNameCell) {
            // Split by comma for multiple test names
      ...
      setExcelTestOptions(Array.from(testNames));
    } catch (error) {
      console.error("Error loading Excel test options:", error);
      setExcelTestOptions([]);
    }
  };
  ...
  // Load Excel test options when modal opens or selected chamber changes
  useEffect(() => {
    if (isOpen && selectedChamber) {
      ...
      loadExcelTestOptions(machineId);
    }
  }, [isOpen, selectedChamber, data]);

  // Load PRODUCT_CODE mapping (SOURCE_ANO_TYPE -> PRODUCT_CODE) from OQC_Form sheet
  const loadProductCodeFromExcel = async (): Promise<Record<string, string>> => {
    try {
      const arrayBuffer = await fetchMasterSheetArrayBuffer();
      const XLSX = await import("xlsx");
      const workbook = XLSX.read(arrayBuffer, { type: "array" });
      const worksheet = workbook.Sheets["OQC_Form"];
      if (!worksheet) return {};
      const jsonData: any[] = XLSX.utils.sheet_to_json(worksheet, { header: 1, defval: "" });
      if (jsonData.length === 0) return {};
      const headers = (jsonData[0] as string[]).map((h) => h.toString().trim().toUpperCase());
      const sourceIdx = headers.findIndex((h) => h === "SOURCE_ANO_TYPE");
      const codeIdx = headers.findIndex((h) => h === "PRODUCT_CODE");
      if (sourceIdx === -1 || codeIdx === -1) return {};
      const map: Record<string, string> = {};
      for (let i = 1; i < jsonData.length; i++) {
        const row = jsonData[i] as any[];
        const src = String(row[sourceIdx] || "").trim();
  ...
  const handlePartScan = async () => {
    ...
    try {
      ...
      if (!productCode && partDetails.processStage) {
        const codeMap = await loadProductCodeFromExcel();
            ...
            <div className="mb-4">
              ...
              <div className="relative">
                {(() => {
                  ...
                  // Show all Excel tests for this machine — the machine is already selected
                  ...
                  const filteredExcelOptions = excelTestOptions;
              ...
              {/* Test Count Info */}
              {excelTestOptions.length > 0 && (() => {
                const count = excelTestOptions.length;
                return (
