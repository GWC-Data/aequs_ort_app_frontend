...
import { fetchMasterSheetArrayBuffer } from "@/lib/backendApi";
...
const PlanningPage = () => {
  ...
  const loadMachineDataFromExcel = async () => {
    try {
      const arrayBuffer = await fetchMasterSheetArrayBuffer();
      const workbook = XLSX.read(arrayBuffer, { type: "array" });

      // Load Machine_Details sheet
      const machineSheetName = "Machine_Details";
      const machineWorksheet = workbook.Sheets[machineSheetName];

      if (!machineWorksheet) {
        console.error("Machine_Details sheet not found in Excel file");
        setError("Machine_Details sheet not found in Excel file");
        return [];
      }

      // Convert sheet to JSON with first row as headers
      const machineJsonData: any[] = XLSX.utils.sheet_to_json(
        machineWorksheet,
        {
          header: 1,
      ...
      if (machineJsonData.length === 0) {
        console.error("Machine_Details sheet is empty");
        setError("Machine_Details sheet is empty");
        return [];
      }

      const headers = machineJsonData[0] as string[];
      console.log("Excel headers found:", headers);

      // Find column indices for equipment ID and machine description
      // Try multiple variations of column names
      const machineIdIndex = headers.findIndex((h) => {
        const lowerHeader = h.toLowerCase().trim();
        return (
          lowerHeader.includes("equipment") ||
          lowerHeader.includes("machine_id") ||
          lowerHeader.includes("machineid") ||
          lowerHeader === "equipment id" ||
          h.trim() === "Equipment ID"
        );
      });
      ...
      const descriptionIndex = headers.findIndex((h) => {
        ...
        return (
          ...
          lowerHeader.includes("machine_description") ||
          lowerHeader === "machine description" ||
          h.trim() === "Machine Description"
        );
      });

      if (machineIdIndex === -1 || descriptionIndex === -1) {
        console.error("Required columns not found in Machine_Details sheet");
        console.log("Headers found:", headers);
        console.log("Machine ID index:", machineIdIndex);
        console.log("Description index:", descriptionIndex);
        setError(
          "Required columns (Equipment ID or Machine Description) not found in Excel",
        );
        return [];
      }

      // Extract machine data (skip header row)
      const machines: MachineItem[] = [];

      for (let i = 1; i < machineJsonData.length; i++) {
        const row = machineJsonData[i] as any[];
        const machineId = row[machineIdIndex];
        ...
        if (machineId && description) {
          machines.push({
            ...
            machine_description: String(description).trim(),
          });
        }
      }

      console.log("Excel machine data loaded:", machines);

      if (machines.length === 0) {
        console.warn("No machine data found in Machine_Details sheet");
        setError("No machine data found in Excel file");
        return [];
      }

      return machines;
    } catch (error) {
      console.error("Error loading Machine_Details from Excel:", error);
      setError("Failed to load machine data from Excel file");
      return [];
    }
  };
  ...
  const initializeData = async () => {
    ...
    try {
      // Load machine data from Excel
      const machinesFromExcel = await loadMachineDataFromExcel();
      console.log("Machines from Excel:", machinesFromExcel);
