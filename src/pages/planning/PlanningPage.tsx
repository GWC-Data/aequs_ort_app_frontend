...
import { RefreshCw, FileSpreadsheet, Calendar, Table, AlertCircle } from 'lucide-react';
...
const PlanningPage = () => {
  ...
  // Selected Items
  const [selectedChamberForTesting, setSelectedChamberForTesting] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (data.length > 0 && chamberLoads.length > 0) {
      ...
      const interval = setInterval(() => {
        const hasActiveLoads = chamberLoads.some(load => 
  ...
  const handleViewMachineDetails = (machine: MachineItem) => {
    ...
    const machineDetails = {
      ...
      currentStatus: getEquipmentStatus(machine.machine_id)?.status || 'available'
    };
  ...
  const handleOpenTestingModal = (machineIdentifier: string) => {
    const machineLoads = chamberLoads.filter(load =>
      ...
      (data.find(m => m.machine_id === machineIdentifier)?.machine_description === load.chamber)
    );

    const activeLoads = machineLoads.filter(load => !isLoadCompleted(load));

    if (activeLoads.length === 0) {
      alert('No active loads found for this equipment');
      return;
    }
  ...
  const handlePauseTimer = async (machineIdentifier: string) => {
    ...
    const machineLoads = chamberLoads.filter(load =>
      (load.chamber === machineIdentifier || load.chamber === machine.machine_description) &&
      !isLoadCompleted(load) &&
    ...
    try {
      ...
      refreshAvailability();
      setError(null);
      alert(`Test paused for ${machineIdentifier} (${machineLoads.length} load(s) affected).`);
    } catch (err) {
  ...
  const handleResumeTimer = async (machineIdentifier: string) => {
    ...
    try {
      ...
      refreshAvailability();
      setError(null);
      alert(`Test resumed for ${machineIdentifier} (${machineLoads.length} load(s) affected).`);
    } catch (err) {
  ...
  const handleStartTimer = async (machineIdentifier: string) => {
    ...
    if (machineLoads.length === 0) {
      alert('No pending tests found to start');
      return;
    }
    ...
    try {
      ...
      refreshAvailability();
      setError(null);
      alert(`Test started for ${machineIdentifier} (${machineLoads.length} load(s) affected).`);
    } catch (err) {
  ...
  return (
    <div className="w-full p-4 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-xl shadow-xl overflow-hidden border border-gray-200">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 border-b bg-gradient-to-r from-blue-50 to-white">
          <div className="flex items-center gap-3 mb-4 sm:mb-0">
            ...
            <div>
              <h2 className="text-2xl font-bold text-gray-800">Equipment Schedule Gantt Chart</h2>
              <p className="text-sm text-gray-600 mt-1">Timeline view of equipment testing schedules</p>
              <div className="flex items-center gap-4 mt-2">
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-green-500 rounded-full"></div>
                  <span className="text-xs text-gray-600">Available: {Object.values(machineAvailability).flatMap(m => Object.values(m)).filter(m => m.status === 'available').length}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-red-500 rounded-full"></div>
                  <span className="text-xs text-gray-600">Occupied: {Object.values(machineAvailability).flatMap(m => Object.values(m)).filter(m => m.status === 'occupied').length}</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-blue-500 rounded-full"></div>
                  <span className="text-xs text-gray-600">Active Loads: {chamberLoads.filter(load => !isLoadCompleted(load)).length}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            {/* Timeline Selector */}
            <div className="flex items-center gap-2">
              <label className="text-sm text-gray-700 font-medium">Timeline:</label>
              <select
                value={numberOfDays}
        ...
        {loading ? (
          <div className="text-center py-16">
            ...
            <p className="mt-4 text-gray-600 font-medium">Loading equipment data...</p>
      ...
      <MachineDetailsModal
        ...
        getEquipmentStatus={getEquipmentStatus}
        chamberLoads={chamberLoads.filter(load => 
