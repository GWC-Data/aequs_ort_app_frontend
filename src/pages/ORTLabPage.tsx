...
//   // In ORTLabPage component, modify to handle same session with multiple statuses:
...
//       newIsReupload ||
//       (hasPreviousEntry && recordId >= existingId);

//     const normalizedDetailBox: DetailBox = {
//       ...detailBox,
//       isReupload: effectiveIsReupload || detailBox.isReupload || existingIsReupload,
//       previousStatus: detailBox.previousStatus ?? existing?.detailsBox.previousStatus,
//     };

//     const resolvedTotalQuantity =
//       parts.length || Number(record.totalQty ?? normalizedDetailBox.totalQuantity ?? 0);
...
//       scannedPartsId: typeof record.Id === "number" ? record.Id : existing?.scannedPartsId,
...
//     const effectiveStatus = isReuploadPending ? { received: "", label: "Pending" } : statusInfo;
...
//         return record.detailBox.partNumbers.filter((value): value is string => Boolean(value && value.trim()));
//       }

//       return existing?.partNumbers ?? [];
//     })();
//     const movedToStage2 = isReuploadPending ? false : Boolean(record.detailBox?.movedToStage2);
...
//       return existing?.partsBeingSent ?? Number(record.allowedParts ?? detailBox.totalQuantity);
//     })();

//     const normalizedDetailBox = {
//       ...detailBox,
//       totalQuantity: partsCount > 0 ? partsCount : detailBox.totalQuantity,
//       previousStatus: detailBox.previousStatus ?? existing?.detailsBox?.previousStatus ?? previousRemarks,
...
//       stage2Enabled:
//         effectiveStatus.received !== "" && effectiveRemarks.trim() !== "" && !movedToStage2,
//       status: effectiveStatus.label,
//       movedToStage2,
//       movedToStage2At: record.detailBox?.movedToStage2At ?? existing?.movedToStage2At,
//       partNumbers: partNumbersFromDetail,
//       totalQuantity: partsCount,
//       scannedPartsId: detailBox.scannedPartsId ?? existing?.scannedPartsId,
//       ortRecordId: typeof record.Id === "number" ? record.Id : existing?.ortRecordId,
//       isReupload: reuploadFlag,
//     };
...
//   const [sessionOrtIdMap, setSessionOrtIdMap] = useState<Record<string, number>>({});
...
//         (row) => row.received !== "Yes" && (!row.movedToStage2 || row.isReupload)
//       );
...
//           await axios.put(`${API_BASE_URL}/ort/${resolvedOrtId}`, payload);
//           updatedRecords.push({ sessionId: sessionKey, ortRecordId: resolvedOrtId });
//         } else {
...
//             updatedRecords.push({ sessionId: sessionKey, ortRecordId: createdId });
//           }
//         }
//       })
//     );

//     const ortIdLookup = new Map(updatedRecords.map((record) => [record.sessionId, record.ortRecordId]));
...
//       setTableData((prev) => prev.filter((row) => !completedSessions.has(row.sessionId)));
